...
def _to_rate(value):
    """
    Convert percentage-like outputs to proportions when needed.
    Existing figure code expects skip/regression rates in [0, 1].
    """
...
def export_eye_movement_pointwise_metrics(df: pd.DataFrame, output_dir: str):
    """
    Export human eye-movement metrics at two granularities:

    1) Trial level:
       one row per participant × stimulus × time condition.

    2) Participant-condition level:
       one row per participant × time condition, averaged across that
       participant's stimuli. This is the preferred file for plotting
       individual dots in the main figure.

    Outputs:
      - human_eye_movement_trial_metrics_pointwise.json/csv
      - human_eye_movement_participant_metrics_pointwise.json/csv
      - human_eye_movement_metrics_participant_aggregated.json
    """
    rows = []

    for (time_constraint, stimulus_id, participant_id), trial_data in df.groupby(
        ['time_constraint', 'stimulus_index', 'participant_id']
    ):
        fixation_data = trial_data.to_dict('records')
        metrics_calculator = AggregatedFixationMetrics(
            fixation_data=fixation_data,
        ...
        reading_speed = trial_metrics.get('Average Reading Speed (wpm)', np.nan)
        skip_rate = trial_metrics.get('Word Skip Percentage by Saccades V2 With Word Index Correction', np.nan)
        regression_rate = trial_metrics.get('Revisit Percentage by Saccades V2 With Word Index Correction', np.nan)

        rows.append({
            'source': 'human',
            'participant_id': participant_id,
            'stimulus_index': int(stimulus_id),
            'time_condition': f'{int(time_constraint)}s',
            'total_time': int(time_constraint),
            'reading_speed': float(reading_speed) if reading_speed is not None and np.isfinite(reading_speed) else float('nan'),
            'skip_rate': _to_rate(skip_rate),
            'regression_rate': _to_rate(regression_rate),
            'raw_metrics': trial_metrics,
        })

    trial_df = pd.DataFrame(rows)
    trial_df = trial_df.sort_values(['participant_id', 'total_time', 'stimulus_index'])

    trial_json = os.path.join(output_dir, 'human_eye_movement_trial_metrics_pointwise.json')
    trial_csv = os.path.join(output_dir, 'human_eye_movement_trial_metrics_pointwise.csv')
    trial_df.to_json(trial_json, orient='records', indent=2)
    trial_df.drop(columns=['raw_metrics']).to_csv(trial_csv, index=False)

    # Average across stimuli for each participant × time condition.
    value_cols = ['reading_speed', 'skip_rate', 'regression_rate']
    participant_df = (
        trial_df
        .groupby(['source', 'participant_id', 'time_condition', 'total_time'], as_index=False)[value_cols]
        .mean()
        .sort_values(['participant_id', 'total_time'])
    )

    participant_json = os.path.join(output_dir, 'human_eye_movement_participant_metrics_pointwise.json')
    participant_csv = os.path.join(output_dir, 'human_eye_movement_participant_metrics_pointwise.csv')
    participant_df.to_json(participant_json, orient='records', indent=2)
    participant_df.to_csv(participant_csv, index=False)

    # Aggregated summary from participant-level means. This is useful for checking
    # against the existing human_eye_movement_metrics.json file and for figure captions.
    agg = {}
    for cond, cond_df in participant_df.groupby('time_condition'):
        agg[cond] = {}
        for metric in value_cols:
            values = cond_df[metric].dropna().to_numpy(dtype=float)
            agg[cond][f'{metric}_mean'] = float(np.mean(values)) if values.size else float('nan')
            agg[cond][f'{metric}_std'] = float(np.std(values, ddof=1)) if values.size > 1 else 0.0
        agg[cond]['num_participants'] = int(cond_df['participant_id'].nunique())
        agg[cond]['num_trials'] = int((trial_df['time_condition'] == cond).sum())

    agg_json = os.path.join(output_dir, 'human_eye_movement_metrics_participant_aggregated.json')
    with open(agg_json, 'w') as f:
        json.dump(agg, f, indent=4)

    print(f'Trial-level eye metrics saved to {trial_json}')
    print(f'Participant-level eye metrics saved to {participant_json}')
    print(f'Participant-aggregated eye metrics saved to {agg_json}')

    return trial_df, participant_df, agg
...
def main():
    ...
    # Export trial-level and participant-level eye-movement metrics for plotting dots
    export_eye_movement_pointwise_metrics(df, output_dir)
