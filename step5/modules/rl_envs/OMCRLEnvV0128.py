...
class OculomotorControllerRLEnv(Env):
    ...
    def __init__(
        ...
        This RL env is for capturing human's features when applying fixations for word activation.
        Needs to implement:
            1. realisitic word activation from a bag of words / embedding / lexicon database ...
            2. could account for the effects of word length, frequency, and predictability ...

        TODO: 
        1. The model itself: integrate the bayesian inference into the belief update. 
            The model should output a list of 5 or 6 words with top probabilities. p(word|letters)=p(word)*p(letters|words), the letters are perceived correctly. 
            But the word could be diversed. It does not have to be the target word (right?). So we could produce a few of words (activated from the bag of words) 
            that competes for recognition. More importantly, the belief could be a probability distribution across many words.
        2. The pipeline: new training and testing dataset (maybe the vocabulary is not needed anymore); a figure shows how's everything working.

        I will first start from a toy model of word activation by sampling.

        Author: Bai Yunpeng

        This version v0128 is derived from v1126, it is a starting point for nature-level paper's agent.
        """
        ...
        print(
            f"{cons.LV_TWO_DASHES} Oculomotor Controller Environment V0128 -- Deploying the environment in the {self._config['rl']['mode']} mode.")
        ...
        self._w_incorrectness_penalty = None
        ...
        self._num_stateful_info_obs = 10 + 2 * self.MAX_WORD_LEN
    ...
    def reset(self, seed=None, inputs=None, ep_idx=None):
        ...
        if inputs is None:
            ...
            # Randomly sample a penalty reward weighting factor as a float
            self._w_incorrectness_penalty = np.random.uniform(0, 1)
        else:
            # Get the assigned penalty reward weighting factor
            self._w_time_penalty = inputs['time_awareness_weight']
            # Get the assigned penalty reward weighting factor
            self._w_incorrectness_penalty = 0
    ...
    def _get_obs(self):
        ...
        norm_target_word_index = self._norm_target_word_idx_on_image
        ...
        norm_incorrectness_penalty = aux.normalise(self._w_incorrectness_penalty, 0, 1, -1, 1)
    ...
    def _compute_reward(
        ...
        static_weight = 100
        time_penalty = 0.02 * static_weight * self._reward_shaping(    # NOTE decrease the reward penalties with larger and more complex environment
    ...
    def _reward_shaping(self, center_x, center_y, fix_x, fix_y, k=0.005, w=0.1):    # NOTE the reward shaping does not help much when the ambiguious parts increase; need to adapt the reward and observation
