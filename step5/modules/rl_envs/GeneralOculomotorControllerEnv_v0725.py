...
class GeneralOculomotorControllerEnv(Env):
    ...
    def __init__(
            self,
            shared_dataset_metadata_of_image_stimuli: dict = None,
            shared_dataset_encoded_lexicon: dict = None,
            ):
        ...
        print(
            f"{cons.LV_TWO_DASHES}General Oculomotor Controller Environment -- Deploying the environment in the {self._config['rl']['mode']} mode.")
        ...
        # Metadata of the image stimuli
        self._metadata = shared_dataset_metadata_of_image_stimuli
        # Encoded lexicon
        self._encoded_lexicon = shared_dataset_encoded_lexicon
        ...
        # TODO, or can we regulate the actions to be only sampled from the bounding box? -- masked actions
    ...
    def reset(self, seed=None, inputs=None, ep_idx=None):
        ...
        # self._image_env_index = 0
        # print(f"Image index: {self._image_env_index}")

        # if ep_idx is not None:
        #     self._image_env_index = ep_idx % 50
        ...
        # # Get the corresponding image pixel values  -- TODO disable this, instead, read from metadata
        ...
        # # TODO debug delete later
        # self._target_word_idx_on_image = 50
        # self._target_word = self._words_on_image[self._target_word_idx_on_image]
        # print(f"Target word index on image: {self._target_word_idx_on_image}, that word is: {self._target_word}")
        # # TODO debug delete later
        # print(f"Be aware, the bbox is: {self._target_word_bbox}")
        ...
        # # TODO check the validity of that
        # print(f"Image pixels shape: {self._normalised_image_env_foveal_patch_pixels.shape}")
        # print(f"Image pixels type: {type(self._normalised_image_env_foveal_patch_pixels)}")
        # print(f"Image pixels: {self._normalised_image_env_foveal_patch_pixels}")
        # return
        ...
        # print(f"Target word masked peripheral view shape: {self._target_word_masked_peripheral_view.shape}")
        # print(f"Target word masked peripheral view type: {type(self._target_word_masked_peripheral_view)}")
        # print(f"Target word masked peripheral view: {self._target_word_masked_peripheral_view}")
        ...
        # Initialize the weighting factor for the penalty reward
        if inputs is None:
            # # Randomly sample a penalty reward weighting factor
            # self._w_time_penalty = np.random.uniform(self._w_time_penalty_lower_bound, self._w_time_penalty_upper_bound)
            # Randomly sample a penalty reward weighting factor as an integer
            self._w_time_penalty = np.random.randint(self._w_time_penalty_lower_bound, self._w_time_penalty_upper_bound + 1)
        else:
            # Get the assigned penalty reward weighting factor
            self._w_time_penalty = inputs['time_awareness_weight']

        # if time_awareness_weight is not None:
        #     self._w_time_penalty = time_awareness_weight
        ...
        self._get_logs_for_test(
            reward=self._compute_reward(done=False, w_time_penalty=self._w_time_penalty),
            done=False,
        )
    ...
    def step(self, action):
        ...
        # Compute the reward
        reward = self._compute_reward(done=done, w_time_penalty=self._w_time_penalty)

        # # Log the fixation for the simulation
        # self.get_logs(reward=reward, done=done)
    ...
    def _get_fix_xy(self):
        fix_x = aux.normalise(self.norm_fix_x, -1, 1, 0, self._original_img_env_width)
        fix_y = aux.normalise(self.norm_fix_y, -1, 1, 0, self._original_img_env_height)
        return fix_x, fix_y
    ...
    def _get_obs(self):
        ...
        if self._target_word_bbox[0] <= self._fix_x <= self._target_word_bbox[2] and self._target_word_bbox[1] <= self._fix_y <= self._target_word_bbox[3]:
            ...
            # If the fixation is not on the target word's bounding box, then the foveal view is all white --  all are 1s
        ...
        norm_penalty_weight = aux.normalise(self._w_time_penalty, self._w_time_penalty_lower_bound, self._w_time_penalty_upper_bound, -1, 1)
        ...
        # # TODO debug show the downsampled foveal view, delete later
        # print(f"Downsampled foveal view shape: {normalised_downsampled_foveal_view_gray_pixels.shape}")
        # print(f"The current fixations are at: {self._fix_x}, {self._fix_y}")
        # plt.imshow(normalised_downsampled_foveal_view_gray_pixels, cmap='gray', vmin=-1, vmax=1)
        # plt.show()
        # print(f"Peripheral view shape: {self._normalised_masked_downsampled_peripheral_view_pixels.shape}")
        # plt.imshow(np.squeeze(self._normalised_masked_downsampled_peripheral_view_pixels), cmap='gray', vmin=-1, vmax=1)
        # plt.show()
        # print(f"Norm internal word: {norm_internal_word}")
        # print(f"Norm word counter: {norm_word_counter}")
        # print(f"Flag on target word: {self._flag_fixate_on_target_word}")
    ...
    def _compute_reward(
            ...
            w_time_penalty: float = 1.0,   # Have changed to a changing weighting factor
            w_completeness: float = 1.0,
            w_bonus: float = 100.0,
    ):

        # Apply a position/distance-based reward
        # time_penalty = 0.02 * w_time_penalty * self._reward_shaping(    # TODO decrease the reward penalties with larger and more complex environment
        ...
        # )
        static_weight = 100
        time_penalty = 0.02 * static_weight * self._reward_shaping(    # TODO decrease the reward penalties with larger and more complex environment
            center_x=self._target_word_center_x,
            center_y=self._target_word_center_y,
            fix_x=self._fix_x,
            fix_y=self._fix_y,
        ...
        # # Add some intermediate positive rewards to incentivize the agent to fixate on the target word
        # if self._flag_fixate_on_target_word is True:
        #     intermediate_reward = 0.25
        # else:
        #     intermediate_reward = 0

        # Make the positive reward more continuous and often
        # if self._flag_target_word_new_letters_updated is True:
        #     completeness_reward = w_completeness * 1
        #     # Change to simple 1 for encourage the agent to fixate on the target word and update new letters
        # else:
        #     completeness_reward = 0

        # Regulate the step-wise rewards
        # r = np.clip(shaped_penalty + completeness_reward, -1, 1)
        # r = np.clip(shaped_penalty, -1, 1)

        # time_penalty = w_shaped_penalty * -1
    ...
    def _get_peripheral_view(
            ...
            bbox=None  # Bounding box in the format [top left x, top left y, bottom right x, bottom right y]
    ):
        """
        Get the peripheral view of the image. If a bounding box is provided, change that entire area to black
        in the original image first, then resize.
        """

        # # Show the original stimuli -- TODO comment later
        # print(f"Print out the original stimuli: {original_stimuli_gray_pixels.shape}")
        # plt.imshow(original_stimuli_gray_pixels, cmap='gray')
        # plt.show()

        if bbox is not None:
            # Extract the coordinates from the bounding box and ensure they are integers
            ...
            original_stimuli_gray_pixels[top_left_y:bottom_right_y, top_left_x:bottom_right_x] = 0

        # Resize the image with the black rectangle to the peripheral view size
        peripheral_view_pixels = np.array(
            Image.fromarray(original_stimuli_gray_pixels).resize((peripheral_view_width, peripheral_view_height)))

        # # Print the peripheral view -- TODO comment later
        # print(f"Print out the peripheral view: {peripheral_view_pixels.shape}")
        # plt.imshow(peripheral_view_pixels, cmap='gray')
        # plt.show()

        return peripheral_view_pixels
    ...
    def _sample_letters_from_pixels(self):
        ...
        # Note: for more dynamical skippings, maybe include more words as adjacent words;
        #   decrease the perception as letters become further away;
        #   code the skipping as a control problem belongs to fixations.
        ...
        # # TODO debug delete later
        # print(f"The target word is: {self._target_word}, \n"
        #       f"The foveal covered letters are: {self._fovea_seen_letters[range_bound_start_check_idx:range_bound_end_check_idx]}, "
        #       f"the parafoveal covered letters are: {self._parafovea_seen_letters[range_bound_start_check_idx:range_bound_end_check_idx]}")
