...
class SupervisoryControllerEnv(Env):
    ...
    def __init__(self):
        ...
        This version aims to make the env more concise, clean, and make the reward weight tunable (by increasing the penalties).
        """
        ...
        print(
            f"{const.LV_TWO_DASHES}RL Supervisory Controller Environment Version 1018 -- Deploying the environment in the {self._config['rl']['mode']} mode.")
        ...
        self._num_stateful_info_obs = 9 + self._max_sentences 
    ...
    def step_part1(self, action):
        ...
        if action == 0:  # Continue reading
            ...
            if action > self._num_read_sentences:
                ...
                # print("**********************************************************************************************************************")
                # print(f"This is a revisit step, the current sentence index is: {self.reading_sentence_index}\n")      
                # print("**********************************************************************************************************************")
    ...
    def _get_obs(self):
        ...
        norm_failure_penalty_weight = aux.normalise(self._failure_penalty_weight, self.MIN_REWARD_WEIGHT, self.MAX_REWARD_WEIGHT, -1, 1)
        norm_exploit_weight = aux.normalise(self._exploit_weight, self.MIN_REWARD_WEIGHT, self.MAX_REWARD_WEIGHT, -1, 1)
        norm_explore_weight = aux.normalise(self._explore_weight, self.MIN_REWARD_WEIGHT, self.MAX_REWARD_WEIGHT, -1, 1)
        ...
        obs = np.array([norm_ep_remain_steps, norm_time_condition, norm_remaining_time, norm_current_sentence_index, norm_appraisal_level_weight, 
                        norm_failure_penalty_weight, norm_exploit_weight, norm_explore_weight, norm_reading_progress, 
    ...
    def _get_valid_actions(self):
        ...
        for i in range(1, self._num_read_sentences + 1):
            valid_actions.append(i)  # Regress to already read sentences
        return valid_actions

    def _update_time_states(self, sentence_index, inputted_num_words_explicitly_fixated_in_sentence=None):        # TODO enable later
        """
        ...
        # print("**********************************************************************************************************************")
        # print(f"The total number of sentences is: {self._total_num_sentences}, the current sentence index is: {sentence_index}\n")      # TODO debug delete later
        # print(f"The inputted num words explicitly fixated in sentence is: {inputted_num_words_explicitly_fixated_in_sentence}\n")      # TODO debug delete later
        # print(f"The time spent in seconds is: {self.time_spent_in_seconds}\n")      # TODO debug delete later
        # print("**********************************************************************************************************************")
    ...
    def _init_reward_weights(self):     # TODO make the explore weight a tunable parameter
        """
        Initialize the reward weight.
        """
        # self._failure_penalty_weight = random.uniform(self.MIN_REWARD_WEIGHT, self.MAX_REWARD_WEIGHT)
        # self._exploit_weight = random.uniform(self.MIN_REWARD_WEIGHT, self.MAX_REWARD_WEIGHT)
        # self._explore_weight = random.uniform(self.MIN_REWARD_WEIGHT, self.MAX_REWARD_WEIGHT)
        
        self._failure_penalty_weight = 0.5       
        # self._exploit_weight = 0.5      
        # self._explore_weight = 0.5       

        # self._explore_weight = random.uniform(self.MIN_REWARD_WEIGHT, self.MAX_REWARD_WEIGHT)
        self._explore_weight = 0.1      # TODO: A fixed value for preliminary simulations
    ...
    def _compute_time_up_reward(self):
        ...
        read_appraisal_levels = [self._appraisal_states[idx]['appraisal_level'] for idx in range(self._num_read_sentences)]
        ...
        num_unread_sentences = self._total_num_sentences - self._num_read_sentences
