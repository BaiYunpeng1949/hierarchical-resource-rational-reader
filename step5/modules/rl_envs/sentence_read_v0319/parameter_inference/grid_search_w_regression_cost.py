import os
import re
import argparse
import numpy as np
import pandas as pd

# =========================================================
# Default paths
# =========================================================
HUMAN_DEFAULT = os.path.join("human_data", "all_words_regression_and_skip_probabilities.csv")
SIM_ROOT_DEFAULT = os.path.join("simulation_data")
SIM_REL_DEFAULT = "all_words_regression_and_skip_probabilities.csv"

# New ambiguity-based files
HUMAN_AMBIG_DEFAULT = os.path.join("human_data", "ambiguity_effect_human_binned.csv")
SIM_AMBIG_REL_DEFAULT = "ambiguity_effect_simulation_binned.csv"

# =========================================================
# Four-panel figure definitions (original word-level plots)
# =========================================================
PANELS_DEF = [
    ("skip_vs_length", "length", "skip_probability", "Word length", "Skip probability", True),
    ("skip_vs_logit_predictability", "logit_predictability", "skip_probability", "Logit predictability", "", False),
    ("skip_vs_log_frequency", "log_frequency", "skip_probability", "Log frequency", "", False),
    ("regression_vs_difficulty", "difficulty", "regression_probability", "Word difficulty", "Regression probability", False),
]

AMBIG_PANELS_DEF = [
    ("regression_vs_ambiguity", "ambiguity_zscore", "avg_regression_probability", "Sentence ambiguity", "Regression probability", False),
    ("skip_vs_ambiguity", "ambiguity_zscore", "avg_skip_probability", "Sentence ambiguity", "Skip probability", False),
]

# =========================================================
# Original scorer terms
# =========================================================
CURVES_FOR_SCORE = [
    ("skip_vs_length",               "length",               "skip_probability"),
    ("skip_vs_logit_predictability", "logit_predictability", "skip_probability"),
    ("skip_vs_log_frequency",        "log_frequency",        "skip_probability"),
    ("regression_vs_difficulty",     "difficulty",           "regression_probability"),
]

# =========================================================
# New ambiguity-based scorer terms
# =========================================================
AMBIG_CURVES_FOR_SCORE = [
    ("regression_vs_ambiguity", "ambiguity_zscore", "avg_regression_probability"),
    # Optional:
    ("skip_vs_ambiguity", "ambiguity_zscore", "avg_skip_probability"),
]

WEIGHT_SLOPE = 1.0
WEIGHT_INTER = 1.0


# =========================================================
# Helpers
# =========================================================
def parse_w(folder_name: str):
    m = re.search(r"w_regression_cost_([0-9]+)p([0-9]+)", folder_name)
    if m:
        return float(f"{m.group(1)}.{m.group(2)}")
    m2 = re.search(r"w_regression_cost_([0-9.]+)", folder_name)
    return float(m2.group(1)) if m2 else None


def _has_cols(df: pd.DataFrame, cols):
    return all(c in df.columns for c in cols)


def _xy_from(df: pd.DataFrame, xcol: str, ycol: str):
    if not _has_cols(df, [xcol, ycol]):
        return np.array([]), np.array([])
    sub = df[[xcol, ycol]].dropna()
    return sub[xcol].values.astype(float), sub[ycol].values.astype(float)


def _overlap(xh, xs):
    if xh.size == 0 or xs.size == 0:
        return None
    lo = max(np.min(xh), np.min(xs))
    hi = min(np.max(xh), np.max(xs))
    if lo < hi:
        return lo, hi
    return None


def _fit_line(x, y):
    if x.size < 2 or y.size < 2:
        return np.nan, np.nan
    slope, intercept = np.polyfit(x, y, deg=1)
    return float(slope), float(intercept)


def loss_slope_intercept(h_human: pd.DataFrame, h_sim: pd.DataFrame, xcol: str, ycol: str) -> float:
    """
    Loss = squared slope difference + squared intercept difference
    computed over overlapping x-range.
    """
    xh, yh = _xy_from(h_human, xcol, ycol)
    xs, ys = _xy_from(h_sim,   xcol, ycol)

    ov = _overlap(xh, xs)
    if ov is None:
        return np.nan

    lo, hi = ov
    hmask = (xh >= lo) & (xh <= hi)
    smask = (xs >= lo) & (xs <= hi)

    xh2, yh2 = xh[hmask], yh[hmask]
    xs2, ys2 = xs[smask], ys[smask]

    sh, bh = _fit_line(xh2, yh2)
    ss, bs = _fit_line(xs2, ys2)

    if np.isnan(sh) or np.isnan(ss):
        return np.nan

    return WEIGHT_SLOPE * (ss - sh) ** 2 + WEIGHT_INTER * (bs - bh) ** 2


# =========================================================
# Folder scoring
# =========================================================
def score_folder(
    sim_csv: str,
    human_csv: str,
    sim_ambiguity_csv: str = None,
    human_ambiguity_csv: str = None,
) -> dict:
    """
    Score one simulation folder using:
    - original word-level metrics
    - new ambiguity-based sentence-level metric(s)
    """
    h = pd.read_csv(human_csv)
    s = pd.read_csv(sim_csv)

    scores = {}
    total_parts = []

    # ----- original metrics -----
    for key, xcol, ycol in CURVES_FOR_SCORE:
        val = loss_slope_intercept(h, s, xcol, ycol)
        scores[key] = val
        total_parts.append(val)

    # ----- new ambiguity metrics -----
    if human_ambiguity_csv is not None and sim_ambiguity_csv is not None:
        if os.path.exists(human_ambiguity_csv) and os.path.exists(sim_ambiguity_csv):
            h_amb = pd.read_csv(human_ambiguity_csv)
            s_amb = pd.read_csv(sim_ambiguity_csv)

            for key, xcol, ycol in AMBIG_CURVES_FOR_SCORE:
                val = loss_slope_intercept(h_amb, s_amb, xcol, ycol)
                scores[key] = val
                total_parts.append(val)
        else:
            for key, _, _ in AMBIG_CURVES_FOR_SCORE:
                scores[key] = np.nan

    total = float(np.nansum(total_parts)) if np.isfinite(np.nansum(total_parts)) else float("inf")
    scores["F_total"] = total
    return scores

def post_plot(
    human_csv: str,
    sim_root: str,
    best_folder: str,
    sim_relpath: str,
    human_ambiguity_csv: str,
    sim_ambiguity_relpath: str,
):
    """
    Create:
    1. original 4-panel figure for word-level metrics
    2. separate ambiguity panels for:
       - regression vs ambiguity
       - skip vs ambiguity
    """
    import importlib.util
    import sys

    here = os.path.dirname(os.path.abspath(__file__))
    plot_path = os.path.join(here, "plot.py")
    if not os.path.exists(plot_path):
        print("plot.py not found; skipping figures.")
        return

    spec = importlib.util.spec_from_file_location("plot_module", plot_path)
    plot_module = importlib.util.module_from_spec(spec)
    sys.modules["plot_module"] = plot_module
    spec.loader.exec_module(plot_module)

    figures_dir = os.path.join(here, "figures")
    os.makedirs(figures_dir, exist_ok=True)

    # -------------------------
    # 1. original 4-panel figure
    # -------------------------
    save_path = os.path.join(figures_dir, "probabilities_four_panel.png")

    panels = []
    for panel_name, x_col, y_col, x_label, y_label, x_integer in PANELS_DEF:
        panels.append({
            "panel_name": panel_name,
            "human_csv": human_csv,
            "sim_csv":   os.path.join(sim_root, best_folder, sim_relpath),
            "x_col": x_col,
            "y_col": y_col,
            "x_label": x_label,
            "y_label": y_label,
            "x_integer": x_integer,
        })

    plot_module.plot_in_a_row(panels, save_path)
    print(f"Saved four-panel figure to: {save_path}")

    plot_module.save_panels_separately(panels, figures_dir, base_name="probabilities")
    print(f"Saved word-level panels to: {figures_dir}")

    # -------------------------
    # 2. ambiguity panels
    # -------------------------
    ambiguity_panels = []
    for panel_name, x_col, y_col, x_label, y_label, x_integer in AMBIG_PANELS_DEF:
        ambiguity_panels.append({
            "panel_name": panel_name,
            "human_csv": human_ambiguity_csv,
            "sim_csv":   os.path.join(sim_root, best_folder, sim_ambiguity_relpath),
            "x_col": x_col,
            "y_col": y_col,
            "x_label": x_label,
            "y_label": '   ',
            "x_integer": x_integer,
        })

    plot_module.save_panels_separately(
        ambiguity_panels,
        figures_dir,
        base_name="ambiguity"
    )
    print(f"Saved ambiguity panels to: {figures_dir}")


# =========================================================
# Main
# =========================================================
def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--human", type=str, default=HUMAN_DEFAULT)
    ap.add_argument("--sim_root", type=str, default=SIM_ROOT_DEFAULT)
    ap.add_argument("--sim_relpath", type=str, default=SIM_REL_DEFAULT)

    # new ambiguity arguments
    ap.add_argument("--human_ambiguity", type=str, default=HUMAN_AMBIG_DEFAULT)
    ap.add_argument("--sim_ambiguity_relpath", type=str, default=SIM_AMBIG_REL_DEFAULT)

    ap.add_argument("--out_csv", type=str, default=None)
    args = ap.parse_args()

    human_csv = os.path.abspath(args.human)
    sim_root = os.path.abspath(args.sim_root)
    sim_rel = args.sim_relpath

    human_ambiguity_csv = os.path.abspath(args.human_ambiguity)
    sim_ambiguity_relpath = args.sim_ambiguity_relpath

    out_csv = args.out_csv or os.path.join(sim_root, "grid_search_w_regression_cost_results.csv")

    rows = []

    for name in sorted(os.listdir(sim_root)):
        if not name.startswith("w_regression_cost_"):
            continue

        sim_csv = os.path.join(sim_root, name, sim_rel)
        if not os.path.exists(sim_csv):
            continue

        sim_ambiguity_csv = os.path.join(sim_root, name, sim_ambiguity_relpath)

        scores = score_folder(
            sim_csv=sim_csv,
            human_csv=human_csv,
            sim_ambiguity_csv=sim_ambiguity_csv,
            human_ambiguity_csv=human_ambiguity_csv,
        )

        w_val = parse_w(name)
        rows.append({"folder": name, "w_regression_cost": w_val, **scores})

    if not rows:
        raise RuntimeError("No simulation folders with the required CSV were found.")

    df = pd.DataFrame(rows).sort_values("F_total", ascending=True)

    os.makedirs(os.path.dirname(out_csv), exist_ok=True)
    df.to_csv(out_csv, index=False)

    best = df.iloc[0]

    print("Best w_regression_cost:", best["w_regression_cost"], "folder:", best["folder"])
    print("Per-metric:")
    for key, _, _ in CURVES_FOR_SCORE:
        v = best.get(key, np.nan)
        out = "NA" if pd.isna(v) else f"{float(v):.6f}"
        print(f"  {key}: {out}")

    for key, _, _ in AMBIG_CURVES_FOR_SCORE:
        v = best.get(key, np.nan)
        out = "NA" if pd.isna(v) else f"{float(v):.6f}"
        print(f"  {key}: {out}")

    print("F_total:", best["F_total"])

    # record best + draw figures
    note_path = os.path.join(os.path.dirname(__file__), "figures", "best_param.txt")
    os.makedirs(os.path.dirname(note_path), exist_ok=True)

    with open(note_path, "w", encoding="utf-8") as f:
        f.write(f"Best w_regression_cost: {best['w_regression_cost']}\n")
        f.write(f"Best folder: {best['folder']}\n")
        for key, _, _ in CURVES_FOR_SCORE:
            v = best.get(key, np.nan)
            f.write(f"{key}: {v}\n")
        for key, _, _ in AMBIG_CURVES_FOR_SCORE:
            v = best.get(key, np.nan)
            f.write(f"{key}: {v}\n")
        f.write(f"F_total: {best['F_total']}\n")

    # post_plot(human_csv, sim_root, best["folder"], sim_rel)
    post_plot(
        human_csv=human_csv,
        sim_root=sim_root,
        best_folder=best["folder"],
        sim_relpath=sim_rel,
        human_ambiguity_csv=human_ambiguity_csv,
        sim_ambiguity_relpath=sim_ambiguity_relpath,
    )


if __name__ == "__main__":
    main()