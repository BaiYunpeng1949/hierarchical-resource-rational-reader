...
class SupervisoryControllerEnv(Env):
    ...
    def __init__(self):
        ...
        print(
            f"=====================================\n"
            f"RL Supervisory Controller Environment Version 1030 -- Deploying the environment in the {self._config['rl']['mode']} mode. \n"
            f"=====================================\n"
            )
        ...
        self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached = None
        ...
        # Appraisal and understanding level representations
        self._read_sentences_sampled_representations = None
        self._unread_sentences_sampled_representations = None
        self._padding_sampled_representations = None
        ...
        # Define the action space -- Continue or Revisit     # TODO get into only two actions, continue or regress, and use the appraisal levels to determine which sentence to regress to.
        self.ACTION_CONTINUE_READING = 0
        self.ACTION_REVISIT = 1
        self.action_space = Discrete(2)   

        # Define the observation space
        self._num_stateful_info_obs = 5 + 2 * self._max_sentences   # TODO maybe revisit later
    ...
    def reset(self, seed: int=None, inputs: dict=None):
        ...
        # Initialize the sentences' representations
        self._read_sentences_sampled_representations, self._unread_sentences_sampled_representations, self._padding_sampled_representations = self._update_understanding_representations_using_bernoulli_appraisal_levels()
    ...
    def step_part1(self, action):
        ...
        # Take the action
        if action == self.ACTION_CONTINUE_READING:  # Continue reading
            # if self.reading_sentence_index < (self._total_num_sentences - 1):  # Have not reached the end of the sentences
            if self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached < self._total_num_sentences:  # Have not reached the end of the sentences
                self.reading_sentence_index = self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached    # Read a new sentence
                reward = self._compute_continue_reward()
            else:   # Reached the end of the sentences -- stay put, do nothing
                reward = -0.1   # Penalize for wasting time but doing nothing
        elif action == self.ACTION_REVISIT:  # Revisit to a certain sentence
            # Check whether the action is valid -- only the sentence that has been read can be regressed to
            if self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached > 0:
                # Randomly sample the sentence to regress to
            ...
            raise ValueError(f"Invalid action: {action}. Must be one of the following: {self.action_space}")
    ...
    def step_part2(self, reward, inputted_read_sentence_appraisal_level=None, 
        ...
        # Update the understanding levels using appraisal levels of sentences
        self._read_sentences_sampled_representations, self._unread_sentences_sampled_representations, self._padding_sampled_representations = self._update_understanding_representations_using_bernoulli_appraisal_levels()
    ...
    def _get_obs(self):
        ...
        norm_reading_progress = aux.normalise(self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached, 0, self._total_num_sentences, -1, 1)
        ...
        norm_individual_sentence_appraisal_level = aux.normalise(self._individual_sentence_appraisal_level, self.MIN_APPRAISAL_LEVEL, self.MAX_APPRAISAL_LEVEL, -1, 1)
        ...
        # Get the understanding levels that are directly related to the final step reward
        understanding_levels = np.concatenate((self._read_sentences_sampled_representations, self._unread_sentences_sampled_representations, self._padding_sampled_representations), axis=0)

        obs = np.array([norm_ep_remain_steps, norm_time_condition, norm_remaining_time, norm_current_sentence_index, #norm_individual_sentence_appraisal_level, 
                        norm_remaining_sentences, 
                        # norm_reading_progress, 
                        *norm_appraisal_levels, *understanding_levels])

        return obs

    def _sample_revisit_sentence_by_appraisal_levels(self):
        """
        Sample the sentence to regress to based on the appraisal levels.
        """
        # Get the appraisal levels of the read sentences
        read_appraisal_levels = [self.appraisal_states[idx]['appraisal_level'] for idx in range(self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached)]
        
        # Set revisit probability to 0 for fully understood sentences (appraisal_level == 1.0)
        revisit_probs = [max(1.0 - appraisal_level, 0.0) for appraisal_level in read_appraisal_levels]

        # Check if all revisit_probs are 0
        if all(prob == 0.0 for prob in revisit_probs):
            if self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached == 1:
                # Only one sentence: assign epsilon to ensure it's sampled
                epsilon = 1e-6
                revisit_probs = [epsilon]
            else:
                # Multiple sentences: assign a small epsilon to each to allow sampling
                epsilon = 1e-6
                revisit_probs = [epsilon for _ in revisit_probs]

        # Normalize the revisit probabilities to sum to 1
        total = sum(revisit_probs)
        if total > 0:
            revisit_probs = [prob / total for prob in revisit_probs]
        else:
            # Fallback: assign uniform probabilities if normalization fails
            num = len(revisit_probs)
            revisit_probs = [1.0 / num for _ in revisit_probs]
        
        # Sample the sentence to regress to
        revisit_sentence_index = np.random.choice(self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached, p=revisit_probs)

        return revisit_sentence_index
    ...
    def _init_sentence_states(self, inputs: dict=None):
        ...
        self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached = 0
        self._num_remaining_sentences = self._total_num_sentences - self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached
    ...
    def _advance_reading_progress(self):
        ...
        self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached += 1
    ...
    def _get_valid_actions(self):
        """
        Returns a list or mask of valid actions based on the current state.
        """
        valid_actions = [0]  # Continue reading is always valid
        for i in range(1, self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached + 1):
            valid_actions.append(i)  # Regress to already read sentences
    ...
    def _compute_appraisal_decrease_due_to_forget(appraisal_level):    
        ...
        """
        # Try a simpler mechanism for forgetting -- Intiution, the better one understands, the less portion they forget
    ...
    def _update_stm_with_just_read_sentence(self, sentence_index):
        ...
        if sentence_index in self._stm:
            ...
            # print(f"sentence_index: {sentence_index}, len(self.appraisal_states): {len(self.appraisal_states)}")        # TODO debug delete later
    ...
    def _compute_continue_reward(self):
        ...
        """
        read_appraisal_levels = [self.appraisal_states[idx]['appraisal_level'] for idx in range(self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached)]
        r_t = 0.1 * sum(read_appraisal_levels)
    ...
    def _update_understanding_representations_using_bernoulli_appraisal_levels(self):
        """
        Get the understanding representations using the appraisal levels.
        """
        # Validate number of unique sentences read
        if self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached < 0:
            raise ValueError(f"Invalid number of read sentences: {self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached}. Must be non-negative.")
        if self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached > self._total_num_sentences:
            raise ValueError(f"Number of read sentences ({self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached}) cannot exceed total number of sentences ({self._total_num_sentences}).")
        if self._total_num_sentences > self._max_sentences:
            raise ValueError(f"Total number of sentences ({self._total_num_sentences}) cannot exceed max sentences ({self._max_sentences}).")

        # Initialize read samples
        if self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached == 0:
            read_appraisal_levels = []
            read_samples = []
        else:
            read_appraisal_levels = []
            for idx in range(self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached):
                try:
                    appraisal_level = self.appraisal_states[idx]['appraisal_level']
                except (IndexError, KeyError) as e:
                    raise ValueError(f"Error accessing appraisal level at index {idx}: {e}")
                if not 0 <= appraisal_level <= 1:
                    raise ValueError(f"Appraisal level {appraisal_level} at index {idx} is not between 0 and 1.")
                read_appraisal_levels.append(appraisal_level)
            # Compute the understanding representations for read sentences
            read_samples = [np.random.binomial(1, appraisal_level) for appraisal_level in read_appraisal_levels]

        # Compute unread samples
        num_unread_sentences = self._total_num_sentences - self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached
        unread_samples = [0] * num_unread_sentences

        # Compute padding samples
        num_padding_samples = self._max_sentences - self._total_num_sentences
        padding_samples = [-1] * num_padding_samples

        # Final assertion
        total_length = len(read_samples) + len(unread_samples) + len(padding_samples)
        assert total_length == self._max_sentences, (
            f"The sum of the understanding representations ({total_length}) should be equal to the max_sentences ({self._max_sentences})."
        )

        return read_samples, unread_samples, padding_samples
    ...
    def _compute_time_up_reward(self):
        ...
        # Get rewards for correctly understanding the read sentences
        assert len(self._read_sentences_sampled_representations) == self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached, f"The length of the read sentences sampled representations {len(self._read_sentences_sampled_representations)} should be equal to the total number of unique read sentences: {self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached}."
        read_reward = sum([1 if self._read_sentences_sampled_representations[idx] == 1 else 1 * self._failure_penalty_weight for idx in range(len(self._read_sentences_sampled_representations))])
        
        # Step 2: Compute penalty for unread sentences
        num_unread_sentences = self._total_num_sentences - self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached
        assert num_unread_sentences == len(self._unread_sentences_sampled_representations), f"The length of the unread sentences sampled representations {len(self._unread_sentences_sampled_representations)} should be equal to the number of unread sentences: {num_unread_sentences}."
        unread_penalty = -1 * self._explore_weight * num_unread_sentences

        termination_reward = 10 * (read_reward + unread_penalty)
    ...
    def get_logs(self):      # TODO make this concise and useful later
        ...
        step_log = {
            ...
            'num_read_sentences': self._num_unique_sentences_read_OR_index_plus_one_farthest_sentece_reached,
