...
class SentenceLevelControllerEnv(Env):
    ...
    def __init__(self):
        ...
        Since simulation results have the issue of incomplete sentences, I plan to retrain the model with another mechanism: the weighted reward function.
        ...
        # self._latest_read_word_index = None
        # Time constraints
        ...
        self.MAX_WEIGHT = 1.0
        self.MIN_WEIGHT = 0.0
        self._time_awareness_weight = None

        # Define the RL env spaces
        self._steps = None
        self.ep_len = const.MAX_NUM_WORDS_PER_SENTENCE
    ...
    def reset(self, seed=None, inputs: dict = None):
        ...
        # self._latest_read_word_index = -1
        ...
        # Initialize the time awareness weight
        self._init_time_awareness_weight(inputs=inputs)
    ...
    def _get_obs(self):
        ...
        # Normalised time awareness weight
        norm_time_awareness_weight = aux.normalise(self._time_awareness_weight, self.MIN_WEIGHT, self.MAX_WEIGHT, -1, 1)

        stateful_info_obs = np.array([norm_ep_remaining_steps, norm_time_constraint_level, norm_reading_progress, norm_next_word_predictability, norm_time_awareness_weight])
    ...
    def _init_time_awareness_weight(self, inputs: dict = None):
        """
        Initialize the time awareness weight.
        """
        if inputs is not None:
            self._time_awareness_weight = inputs["time_awareness_weight"]
        else:
            self._time_awareness_weight = random.uniform(self.MIN_WEIGHT, self.MAX_WEIGHT)
    ...
    def have_words_to_skip(self):
        ...
        """
        # Check if there is at least one word to skip over
    ...
    def _skip_next_word(self, predictability: float = None):
        ...
        if skipped_word_index < self.num_words_in_sentence:
            ...
            self._predictability_states[skipped_word_index] = (last_word_predictability_level, self.SKIP_ACTION)
        if read_word_index is not None and read_word_index < self.num_words_in_sentence:
            self._predictability_states[read_word_index] = (1.0, self.READ_ACTION)
    ...
    def _compute_step_reward(self):
        ...
        """
        return self._time_awareness_weight * -1
    ...
    def _compute_terminate_reading_reward(self):
        ...
        aggregated_predictability_values = sum([1 if sample == 1 else -1 for sample in samples])        # TODO think whether we need to average this value later, the size of it will affect the amplitude of time penalties
    ...
    def get_logs(self):
        ...
        step_log = {
            ...
            'time_awareness_weight': self._time_awareness_weight,
            'num_words_read_is_same_as_num_words_in_sentence': self.num_words_read_in_sentence == self.num_words_in_sentence,
        }
        ...
        print(f"The time awareness weight: {self._time_awareness_weight}.\n")
        print(f"The number of words read is the same as the number of words in the sentence: {self.num_words_read_in_sentence == self.num_words_in_sentence}.")
