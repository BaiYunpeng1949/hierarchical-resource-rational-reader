...
class SupervisoryControllerEnv(Env):
    ...
    def __init__(self):
        ...
        """
        Create on 01 November 2024 -- version 2
        ...
        Based on v1101, just see whether the agent could prioritize reading the remaining sentencs. 1. Try apply the step-wise sentences as negative values; 2. or try bigger penalties with the time_up_reward.
        """
        ...
        print(
            f"=====================================\n"
            f"RL Supervisory Controller Environment Version 1101 v2 -- Deploying the environment in the {self._config['rl']['mode']} mode. \n"
            f"=====================================\n"
            )
    ...
    def _sample_revisit_sentence_by_appraisal_levels(self):     
        """
        Sample the sentence to revisit based on the appraisal levels using weighted probabilities.
        """
        ...
        # Convert appraisal levels to weights (lower appraisal levels have higher weights)
        max_level = max(appraisal_levels)
        weights = [max_level - level + 1e-6 for level in appraisal_levels]  # Adding a small epsilon to avoid zero weight
        # Normalize weights to sum to 1
        total_weight = sum(weights)
        probabilities = [weight / total_weight for weight in weights]
        # Randomly select an index based on the probabilities
        sentence_index = np.random.choice(
            range(self._num_unique_sentences_read_OR_farthest_sentence_reached),
            p=probabilities
        )
    ...
    def _get_obs(self):     
        ...
        norm_individual_sentence_appraisal_level = aux.normalise(reading_sentence_appraisal_level, self.MIN_APPRAISAL_LEVEL, self.MAX_APPRAISAL_LEVEL, -1, 1)  
    ...
    def _calculate_appraisal_level(self, sentence_index, operation):
        ...
        if operation == const.SENTENCE_STATES_IN_MEMORY['revisited']:
            ...
            _new_level = 1.0        # try the simple one here to encourage revisits
    ...
    def _compute_continue_read_new_sentence_reward(self):       
        """
    ...
    def _compute_time_up_reward(self):
        ...
        unread_sentences_penalty = -2 * num_unread_sentences
