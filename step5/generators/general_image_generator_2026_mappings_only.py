...
class ImageGenerator:
    ...
    def _generate_simulation_position_metadata(self, json_file: str):
        """
        Generate a compact metadata file for simulation stimuli only.

        This does not save train/test data, rendered images, foveal patches,
        peripheral views, original image pixels, or letter-level metadata.
        It only records the mapping between each stimulus word index and its
        pixel position on the stimulus canvas, which is enough for computing
        saccade amplitudes.
        """
        if not self._corpus_stimulus_sentences_dict:
            raise ValueError("corpus_sentences_dict is required for simulation metadata generation.")
        ...
        metadata = {
            "config": {
                "domain": cons.SIMULATE,
                "metadata_type": "word_index_to_pixel_position_mapping",
                "corpus_file_name": self._corpus_sentences_file_name,
                "image_size_px": list(self._image_size),
                "word_size": self._word_size,
                "left_margin_px": cons.LEFT_MARGIN,
                "right_margin_px": cons.RIGHT_MARGIN,
                "line_spacing_px": cons.LINE_SPACING,
                "num_stimuli": len(self._corpus_stimulus_sentences_dict),
            },
            "stimuli": []
        }

        for stimulus_idx in range(len(self._corpus_stimulus_sentences_dict)):
            sentence = self._corpus_stimulus_sentences_dict[stimulus_idx + 1]
            words_in_stimulus = sentence.split()

            # Use an in-memory image only to keep the text layout identical to
            # the original generator. No image file is saved.
            img = Image.new("L", self._image_size, 255)
            draw = ImageDraw.Draw(img)

            lines, line_height, num_lines, _ = self.split_text_into_lines(
                words_in_stimulus, draw, font, drawable_width, self._image_size[1],
            )
            ...
            total_height = self._get_total_height(max_height=max_font_height, num_lines=num_lines)
            x_init = cons.LEFT_MARGIN + (drawable_width - max_line_width) / 2
            y_init = (self._image_size[1] - total_height) / 2

            word_position_mapping = []
            word_idx_in_stimulus = 0
            ...
            for line_idx, line in enumerate(lines):
                ...
                for word_idx_in_line, word in enumerate(words_in_line):
                    ...
                    word_bbox_px = self._get_bounding_boxes(
                        word_width=word_width,
                        line_height=line_height,
                        word_index_in_line=word_idx_in_line,
                        max_word_index_in_line=len(words_in_line) - 1,
                        x=x,
                        y=y,
                    )
                    ...
                    word_position_mapping.append({
                        "word_index": word_idx_in_stimulus,
                        "word": word,
                        "lexicon_index": self._words_index_dict.get(word),
                        "line_index": line_idx,
                        "word_index_in_line": word_idx_in_line,
                        "word_bbox_px": [float(v) for v in word_bbox_px],
                        "word_center_px": [
                            float((word_bbox_px[0] + word_bbox_px[2]) / 2),
                            float((word_bbox_px[1] + word_bbox_px[3]) / 2),
                        ],
                        "text_bbox_px": [float(v) for v in text_bbox_px],
                        "text_center_px": [
                            float((text_bbox_px[0] + text_bbox_px[2]) / 2),
                            float((text_bbox_px[1] + text_bbox_px[3]) / 2),
                        ],
                    })

                    x += word_width + self._space_width
                    word_idx_in_stimulus += 1

            metadata["stimuli"].append({
                "stimulus_index": stimulus_idx,
                "sentence": sentence,
                "num_words": len(words_in_stimulus),
                "num_lines": num_lines,
                "x_init_px": float(x_init),
                "y_init_px": float(y_init),
                "words": word_position_mapping,
            })

        with open(json_file, "w", encoding="utf-8") as f:
            json.dump(metadata, f, indent=2, ensure_ascii=False)

        print(f"Compact simulation position metadata saved to {json_file}")

    def generate_data(self):
        """Generate only compact simulation metadata for word-position mapping."""
        current_date_time = datetime.datetime.now().strftime("%m_%d_%H_%M")
        folder_name = (
            f"{current_date_time}_simulation_word_positions_"
            f"W{self._image_size[0]}H{self._image_size[1]}"
            f"_WS{self._word_size}_LS{int(cons.LINE_SPACING)}_MARGIN{cons.LEFT_MARGIN}"
        )
        save_path = os.path.join(self._json_dir, folder_name)
        os.makedirs(save_path, exist_ok=True)

        json_sim_file = os.path.join(save_path, "simulation_word_position_metadata.json")
        self._generate_simulation_position_metadata(json_file=json_sim_file)
