...
def _scatter_values(ax, x, values, color, bar_w):
    """Overlay point-wise values on a bar without changing the bar/axis layout."""
    vals = [float(v) for v in values if v is not None]
    if not vals:
        return

    # Deterministic horizontal jitter, contained within the bar width.
    # This avoids random movement across repeated exports.
    jitter_half_width = bar_w * 0.32
    if len(vals) == 1:
        jitter = np.array([0.0])
    else:
        jitter = np.linspace(-jitter_half_width, jitter_half_width, len(vals))
...
def _bar_group(ax, centers, means_h, stds_h, means_s, stds_s,
               values_h=None, values_s=None, ylabel=None, xlabels=None):
    ...
    # Overlay individual participant / simulation-run values if present in the JSON.
    if values_h is not None:
        for x, vals in zip(offsets_h, values_h):
            _scatter_values(ax, x, vals, HUMAN_COLOR, bar_w)
    if values_s is not None:
        for x, vals in zip(offsets_s, values_s):
            _scatter_values(ax, x, vals, SIM_COLOR, bar_w)
...
def main():
    ...
    for i, (metric_key, ylabel) in enumerate(metrics):
        ...
        h_values = [data["human"][metric_key][c].get("values", []) for c in conditions]
        s_values = [data["simulation"][metric_key][c].get("values", []) for c in conditions]

        rects_h, rects_s = _bar_group(ax, centers, h_means, h_stds, s_means, s_stds,
                                      values_h=h_values, values_s=s_values,
