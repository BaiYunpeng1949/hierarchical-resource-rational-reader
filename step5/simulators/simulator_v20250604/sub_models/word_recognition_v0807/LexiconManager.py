...
class LexiconManager():
    ...
    TODO: something different -- when simulating, we need actual words, not made-up words as targets, but the parallel stuff could still be artificial; 
    and I need to set the priors properly: either only use one or use both in an integral way    
    """

    def __init__(self, mode=None, zipf_param=1.5):
    ...
    def get_likelihood_by_sampled_letters_so_far(
        ...
        if candidate_word == original_word:
            ...
            # dist = Utilities.levenshtein_distance(candidate_word, original_word)
    ...
    def get_a_generated_word(self):
        """
        Randomly generate a word and its corresponding length, freq, and pred
        (this is only called when not simulating)
        """
        ...
        prior_prob, raw_occurance_of_target_word = self._generate_prior_probability()

        # Update the prior dict
        self.update_prior_dict(word_to_recognize=word_to_recognize, prior_prob=prior_prob)

        return word_to_recognize, self.prior_dict[word_to_recognize], raw_occurance_of_target_word
    
    def update_prior_dict(self, word_to_recognize, prior_prob):
        """
        Update the prior dict.
        Called both inside the LexiconManager and outside.
        """
        self.prior_dict[word_to_recognize] = prior_prob