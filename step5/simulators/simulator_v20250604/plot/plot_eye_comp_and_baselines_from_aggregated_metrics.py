...
# -*- coding: utf-8 -*-
"""
plot_eye_comp_comparison_baselines_clipped.py
"""
...
OUT_PNG       = Path("comparison_panel_baselines_clipped.png")
...
def _bar_cluster(ax, centers, series, colors, edges, hatches, ylab=None, xlabels=None, metric_key=None):
    n_series = len(series)
...
def _extract_metric(d, group, metric_key, conditions):
    return {"mean": [d[group][metric_key][c]["mean"] for c in conditions],
...
def main():
    ...
    row1 = [("reading_speed", "Reading speed (WPM)"),
            ("skip_rate", "Skip rate"),
            ("regression_rate", "Regression rate")]
    row2 = [("mcq_accuracy", "MCQ accuracy"),
    ...
    gs = gridspec.GridSpec(nrows=3, ncols=5, figure=fig,
    ...
    def series_for(metric_key):
        ...
        for i,v in enumerate(variants):
            s = {"mean":[data_base["baselines"][v][metric_key][c]["mean"] for c in conditions],
