...
def prepare_fixation_series(
    ...
    If noise_sigma_px > 0, add Gaussian noise N(0, noise_sigma_px^2) in pixels
    to the y-coordinate of each fixation (once, before animation).
    """
    fixations = extract_fixations(trial)
    if not fixations:
        return [], [], [], []
    ...
    labels, is_reg_fix = classify_saccades_by_rules(fixations)
    xs = [f["x"] for f in fixations]
    ys_base = [f["y"] for f in fixations]
...
def main():
    ...
    ap.add_argument("--fps", type=int, default=5,
                    help="Frames per second for both videos (default: 5).")
    ap.add_argument("--human_y_offset_px", type=float, default=0.0,
                    help="Vertical pixel offset for human fixations (default: 0).")
    ap.add_argument("--sim_y_offset_px", type=float, default=0.0,
                    help="Vertical pixel offset for simulation fixations (default: 0).")
    ...
    # RNG for noise (shared so human & sim noise are reproducible together)
    rng = np.random.default_rng(args.noise_seed)

    # Prepare fixation series (with optional y-noise)
    human_series = prepare_fixation_series(
        human_trial,
        args.human_y_offset_px,
        noise_sigma_px=args.noise_sigma_px,
        rng=rng,
    )
    sim_series = prepare_fixation_series(
        sim_trial,
        args.sim_y_offset_px,
        noise_sigma_px=args.noise_sigma_px,
        rng=rng,
    )
